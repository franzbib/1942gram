...
export default function EndScreen({
  ...
  const exportText = [
    ...
    `Mots péjoratifs détruits: ${result.pejorativesDestroyed}`,
    ...
    <main className="end-screen">
      <section className="result-panel">
        ...
        <div className="review-grid">
          <div>
            <h2>Actions clés</h2>
            <p>Péjoratifs détruits : {result.pejorativesDestroyed}</p>
          ...
          <div>
            ...
            {(result.review.slice(0, 5).length ? result.review.slice(0, 5) : []).map((entry) => (
